...
namespace Omegaalfa\Utils\EnvLoader;
...
final class EnvLoader
{
    ...
    public static function get(string $key, ?string $default = null): ?string
    {
        ...
        if (array_key_exists($key, $_ENV)) {
            return self::stringify($_ENV[$key], $key);
        }
        $value = getenv($key);
        if ($value !== false) {
            return $value;
        ...
        if (array_key_exists($key, $_SERVER)) {
            return self::stringify($_SERVER[$key], $key);
        }
        return $default;
    }
    ...
    private static function resolvePath(?string $path): string
    {
        ...
        $envPath = $path ?? ($workingDirectory === false ? '.env' : $workingDirectory . '/.env');
    ...
    private static function stringify(mixed $value, string $key): string
    {
        ...
        throw new RuntimeException("Environment variable {$key} must contain a scalar value.");
    }
}
